...
from src.utils.utils import set_seeds, make_env, get_mask, encode_state, ReplayBuffer, TransitionSARSA
from src.utils.models import QNetwork, linear_schedule



def run_sarsa(args, use_tensorboard=False, use_wandb=False):
    ...
    while episode_number < args.max_episodes:
        ...
        with torch.no_grad():
            target_q = q_network(data.next_state).gather(1, data.next_action).squeeze()
            td_target = data.reward.flatten() + args.gamma * target_q * (1 - data.done.flatten())
