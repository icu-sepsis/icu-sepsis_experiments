"""
Swarm2 Specfici : This file is an attempt to start working on creating a shared file with all commands. 
...
parser.add_argument('--ntasks', '-n', type=float, default = 100) # number of tasks per CPU
...
for json_file in json_files:
    ...
    cpus = int(args.cpus * args.ntasks)
    memory = args.memory
...
# lets create a single file with all the commands
foldername = './temp/parallel_script/'
# foldername = './temp/parallel_scripts/'
if not os.path.exists(foldername):
        os.makedirs(foldername, exist_ok=True)
filename = [f'./temp/parallel_script/node_{i}_{str(args.cpus)}_{str(np.random.randint(0,100000))}.txt' for i in range(nodes) ]
# specify a single file.
filename = f'./temp/parallel_script/node_{str(args.cpus)}.txt'
...
fil.close()
    ...
    #                 f"#SBATCH --nodes=1\n" \
    #                 f"export TMPDIR=/mnt/nfs/scratch1/dgupta\n"\
    #                 f"source ~/.bashrc\n"\
    #                 f"conda activate cv\n" \
    #                 f"cd {cwd}\n" \
...
# f'export PATH="/mnt/nfs/home/dgupta/miniconda3/bin:$PATH"\n'\
